...
| `nativeMessaging` | The extension's only correspondent is the Foxl Desktop app on the same computer, and this is how it reaches it: `chrome.runtime.connectNative('ai.foxl.browser_bridge')` in `src/service-worker.js`. It replaces a WebSocket to `127.0.0.1`, and it is a SECURITY improvement rather than a new capability - over a local socket the desktop could only identify its caller by the `Origin:` header, which any other program on the machine can forge, so the desktop had to fall back to making the user copy a 64-character pairing secret into the browser. Native messaging lets Chrome vouch for the extension's identity instead (`allowed_origins` in the host manifest, which cannot contain wildcards), so no secret is stored in the browser and no local port is opened. No new data is read or sent, and no remote host is involved: the host process is part of the desktop app the user installed. |
